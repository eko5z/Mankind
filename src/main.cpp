...
int main()
{
	...
	graphics_manager.AddMesh("tree", std::make_unique<QuadMesh>(nullptr, nullptr));
	graphics_manager.AddTexture("tree_diffuse", std::make_unique<Texture>("res/tex/tree.png"));
	graphics_manager.AddProgram("billboard", std::make_unique<Program>("res/shaders/billboard.vert", "res/shaders/billboard.frag"));
